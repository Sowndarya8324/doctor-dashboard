...
app.get("/api/summary", (req, res) => {
  ...
  const alerts = motherNotifications.filter((n) => !n.isRead).length;
...
app.get("/api/patients", (req, res) => {
  const enriched = patients.map((p) => {
    const patientPrescriptions = prescriptionsStore.filter(
      (rx) => rx.patientId === p.id
    );

    const latestRx =
      patientPrescriptions.length > 0 ? patientPrescriptions[0] : null;

    const birthRecord = birthsStore.find(
      (b) => patientToMother[p.id] && b.motherId === patientToMother[p.id]
    );

    return {
      ...p,
      babyId: birthRecord?.babyId || "",
...
function handlePrescription(req, res) {
  ...
  const motherId = patientToMother[patientId] || "M001";
  ...
  motherNotifications.unshift({
    ...
    message: `Doctor prescribed ${medicine} (${dosage})`,
    isRead: false,
    createdAt: nowISO(),
  });
...
app.post("/api/birth", (req, res) => {
  ...
  motherNotifications.unshift({
    ...
    message: `Birth registered successfully. Baby ID: ${babyId}`,
    isRead: false,
    createdAt: nowISO(),
  });
...
app.post("/api/vaccinations/remind", (req, res) => {
  ...
  if (!finalBabyId) {
    const motherId = patientToMother[patientId];
    const latestBirth = birthsStore.find((b) => b.motherId === motherId);
    if (latestBirth) finalBabyId = latestBirth.babyId;
  }
  ...
  motherNotifications.unshift({
    ...
    message: `${v.vaccine} due (${v.when})${note ? `. Note: ${note}` : ""}`,
    isRead: false,
    createdAt: nowISO(),
  });
...
app.get("/api/debug/mother-notifications", (req, res) => {
